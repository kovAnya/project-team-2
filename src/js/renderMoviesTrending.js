...
export   async function renderMoviesTrending(dataFromServer){
        try{
            ...
            if(release_date){                             /// Если в данных есть значение release_date то данный год берем с него
            ...
            for(const ids of genre_ids) {
                if(getMovieGenres(ids)){              /// Если id жанра есть в списке жанров то добавляем в карточку, если нет то не добавляем
                genre.push(` ${getMovieGenres(ids)}`)
                }
             }


            if(title) {                      /// Название фильма берем с title 
